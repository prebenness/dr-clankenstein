...
static void fail(hipError_t err, const char *expr, const char *file, int line) {
    std::fprintf(stderr, "HIP error at %s:%d: %s: %s\n", file, line, expr, hipGetErrorString(err));
    std::exit(1);
}

#define CHECK(expr) do { hipError_t err__ = (expr); if (err__ != hipSuccess) fail(err__, #expr, __FILE__, __LINE__); } while (0)

int main() {
    int count = 0;
    CHECK(hipGetDeviceCount(&count));
    if (count < 1) {
        std::fprintf(stderr, "No HIP devices visible\n");
        return 1;
    }
    ...
    std::printf("{\"ok\":%s,\"backend\":\"hip\",\"device_count\":%d,\"device_name\":", ok ? "true" : "false", count);
    json_string(prop.name);
    std::printf(",\"n\":%d,\"max_abs_error\":%.9g,\"node\":", N, max_abs_error);
