...
Options:
  --gpus N                GPUs per node. Default: from agent.sbatch, else 1
  --cpus N                CPUs per task. Default: from agent.sbatch, else 1
  --time HH:MM:SS         Wall time. Default: from agent.sbatch, else 01:00:00
...
if (( verbose )); then
    ...
    printf '\nUse --verbose for pending-job details and sbatch --test-only output.\n'
fi
